...
def extract_quaternion_sign_change(quaternion_sequence: torch.Tensor, device: str):
    """
    Ensure that quaternions in a time-sequence are in same hemisphere.

    Args:
        quaternion_sequence: quaternions sequence with real part first,
            as tensor of shape (..., T, 4).

    Returns:
        signs: signs of quaternions. 
    """
    if len(quaternion_sequence.shape) > 2:
        pad = torch.zeros((*quaternion_sequence.shape[:-2], 1, 4), device=device)
    else:
        pad = torch.zeros((1, 4), device=device)

    curr_q: torch.Tensor = torch.concat((pad, quaternion_sequence), axis=-2)
    prev_q: torch.Tensor = torch.concat((quaternion_sequence, pad), axis=-2)

    if len(quaternion_sequence.shape) > 2:
        signs = torch.einsum('ijkl,ijlk->ij', prev_q.unsqueeze(-2), curr_q.unsqueeze(-1))
    else:
        signs = torch.einsum('ijk,ikj->i', prev_q.unsqueeze(-2), curr_q.unsqueeze(-1))
        
    signs = signs<0

    kernel = signs[..., 0]
    for t in range(1, signs.shape[-1]):
        if signs[..., t].sum() > 0:
            kernel = signs[..., t].logical_xor(kernel)
        
        signs[..., t] = kernel.logical_or(signs[..., t]) 

    return torch.logical_and(signs[..., 1:], signs[..., :-1])


def ensure_quaternion_consistency(quaternion_sequence: torch.Tensor, device:str):
    """
    Ensure that quaternions in a time-sequence are in same hemisphere.
    ...
    Returns:
        quaternion_sequence: quaternions sequence with real part first,
            as tensor of shape (..., T, 4). 
            All the quaternions are in same hemisphere
    """
    
    signs = extract_quaternion_sign_change(quaternion_sequence, device)
    quaternion_sequence[signs] *= -1

    return quaternion_sequence


def mirror_quaternion(quaterion_sequence: torch.Tensor, axis: str):
    quat = quaterion_sequence.clone()
    if 'x' in axis.lower():
        quat[..., :, 1] *= -1
    if 'y' in axis.lower():
        quat[..., :, 2] *= -1
...
def extract_principal_rotation(joint_trajectory: torch.Tensor, axis: str, device: str) -> torch.Tensor:
        traj_length = joint_trajectory.shape[0]
        if axis.lower() == 'x':
            axis_vector = torch.tensor([1.0, 0.0, 0.0], device=device).repeat(traj_length, 1)
        elif axis.lower() == 'y':
            axis_vector = torch.tensor([0.0, 1.0, 0.0], device=device).repeat(traj_length, 1)
        elif axis.lower() == 'z':
            axis_vector = torch.tensor([0.0, 0.0, 1.0], device=device).repeat(traj_length, 1)
        else:
...
def extract_rotation_from_quaternion(quaternion_trajectory: torch.Tensor):
    '''
